...
def set_up_board(board_size, human_plays=False):
    empty_board = np.array([[' '] * board_size**2]).reshape(board_size, board_size)
...
def find_empty_fields(state):
    empty_cells = [(i, j) for i in range(board_size) for j in range(board_size) if state[i, j] == ' ']
    return empty_cells
...
def print_board(state):
    ...
    col_num_to_print = [str(i+1) for i in range(board_size)]
